...
def analyze_stock(row, strategy_mode):
    try:
        ...
        # 3. 이격도 (20일선 대비 15% 이상 급등 제외)
        ma20 = df['MA20'].iloc[-1]
        if not pd.isna(ma20):
            if (curr - ma20) / ma20 * 100 > 15.0: return None

        # 4. 역배열 120일선 저항 (머리 위 1% 이내)
        ...
        if strategy_mode == '0':
            ...
            score_str = "🐣단밤 칼지지 (침범X)"
            rec_entry = int(black)
            stop_loss = int(black * 0.99)
            ...
            score_str = "🛫이륙준비 (정배열초기)"
            rec_entry = int(curr)
            stop_loss = int(min(ma112, black)*0.97)
...
def main():
    ...
    with st.sidebar:
        ...
        st.markdown("### 📈 전략 선택")
        # [수정] selectbox(드롭다운) -> radio(전체 펼침 목록)으로 변경
        strategies = {
            '0': '0. 🐣 단밤 지지 (무관용 원칙)',
            '1': '1. 💎 최바닥주 (찐바닥)',
            '2': '2. 🚀 눌림목 (가짜 제거)',
        ...
        try:
            ...
            if not results:
                ...
                for i, row in df_res.iterrows():
                    with st.expander(f"[{row['시장']}] {row['종목명']} ({row['코드']}) - {row['점수']}"):
                        col1, col2 = st.columns([1, 2])
                        
                        with col1:
                            st.markdown(f"""
                            - **현재가:** {int(row['현재가']):,}원 ({row['등락률']}%)
                            - **거래대금:** {int(row['거래대금']/100000000)}억
                            - **RSI:** {row['RSI']}
                            - **진입가:** <span style='color:green'>{int(row['추천진입가']):,}원</span>
                            - **목표가:** <span style='color:red'>{int(row['목표가']):,}원</span>
                            - **손절가:** <span style='color:blue'>{int(row['손절선']):,}원</span>
                            """, unsafe_allow_html=True)
                            
                            naver_url = f"https://finance.naver.com/item/main.naver?code={row['코드']}"
