...
@st.cache_resource
def install_font():
    # GitHub에 업로드한 폰트 파일 이름 (정확해야 함)
    font_file = 'NanumGothic.ttf' 
    
    # 1. 현재 폴더에 폰트 파일이 있으면 강제 적용 (가장 확실함)
...
st.markdown("##### 👉 차트에 목표가/손절가 라인, 매물대, 폰트 완벽 적용됨")
st.markdown("---")
...
# 분석 실행 로직
if run_btn:
    st.info("🔎 주식 시장을 스캔하는 중입니다... (약 2~3분 소요)")
    try:
        ...
        target = df_krx.sort_values('Marcap', ascending=False).head(1200) # 검색 범위
        ...
        if results:
            ...
            st.success(f"✅ 검색 완료! 총 {len(df_res)}개 종목이 발견되었습니다.")
        else:
...
if st.session_state.result_df is not None and not st.session_state.result_df.empty:
    ...
    st.subheader(f"📊 검색 결과 리스트 ({len(df)}개)")
    ...
    # 차트 시각화 섹션
    st.subheader("📈 차트 상세 분석 (매물대 & 목표가 포함)")
    
    # 선택 상자
    selected_stock = st.selectbox("종목을 선택하면 차트가 그려집니다", df['종목명'] + " (" + df['코드'] + ")")
    ...
    if selected_stock:
        ...
        with st.spinner('차트 그리는 중...'):
            try:
                ...
                # 1. 목표가(Red) & 손절가(Blue) 라인
                if row['목표가'] > 0:
                    ax1.axhline(row['목표가'], color='red', linestyle='-.', linewidth=1.5, alpha=0.8)
                    ax1.text(last_date, row['목표가'], f" 목표 {row['목표가']:,}", color='red', fontweight='bold', va='bottom', fontproperties=FONT_PROP, fontsize=11)
                
                if row['손절선'] > 0:
                    ax1.axhline(row['손절선'], color='blue', linestyle='-.', linewidth=1.5, alpha=0.8)
                    ax1.text(last_date, row['손절선'], f" 손절 {row['손절선']:,}", color='blue', fontweight='bold', va='top', fontproperties=FONT_PROP, fontsize=11)

                # 2. 매물대 (Volume Profile)
                bins = np.linspace(df_chart['Close'].min(), df_chart['Close'].max(), 60)
                prof, _ = np.histogram(df_chart['Close'], bins=bins, weights=df_chart['Volume'])
                axv = ax1.twiny()
                axv.barh(bins[:-1], prof, height=bins[1]-bins[0], alpha=0.2, color='orange') # 투명도 조절
                axv.set_xticks([]); axv.set_xlim(0, max(prof)*2.5) # 매물대 너무 길지 않게
                axv.text(max(prof), bins[prof.argmax()], " 최다매물대", color='darkorange', fontweight='bold', fontproperties=FONT_PROP)
                ...
                if '포착' in score_str or '분홍' in score_str:
                    ...
                    for i in range(1, 30):
                        ...
                        if cond:
                            ax1.scatter(df_chart.index[idx], df_chart['Low'].iloc[idx]*0.97, color='magenta', marker='^', s=200, zorder=10)
                            found_any = True
                    if found_any:
                        ax1.plot(df_chart.index, df_chart['MA60'], color='green', alpha=0.5)
                        ax1.plot(df_chart.index, df_chart['MA88'], color='orange', linewidth=1.5)

                elif '단밤' in score_str:
                    ax1.plot(df_chart.index, df_chart['Breakout_Line'], color='black')
                    ax1.plot(df_chart.index, df_chart['Danbam_Gray'], color='gray', linestyle=':')
                    ax1.scatter(last_date, df_chart['Close'].iloc[-1], color='red', s=150, marker='*')
                
                elif row['trend_info']:
                    ti = row['trend_info']
                    ax1.plot([ti['p1_date'], last_date], [ti['p1_val'], ti['resistance']], color='red', linewidth=2)
                
                else:
                    ax1.plot(df_chart.index, df_chart['Blue_Line'], color='blue', linestyle='--')

                # 타이틀 및 설정
                ax1.set_title(f"{row['종목명']} ({code}) - {score_str}", fontsize=16, fontweight='bold', fontproperties=FONT_PROP)
